...
//   } else if (!filename.includes('AL') && congress < congresses[congresses.length - 1]) {
...
const createNotAL = function (congress) {
  const outfile = (exclude[congress].length > 0) ? `${congress}-notAL.json` : `${congress}.json`;
  ...
  mapshaper.runCommands(mapshaperOptions.join(' '), (err, output) => {
    if (err) {
      console.error(err);
    } else if (exclude[congress].length > 0) {
      createAL(congress);
    } else {
      findExclusions(congress + 1);
    }
...
const createAL = function (congress) {
  ...
  mapshaper.runCommands(mapshaperOptions2.join(' '), (err, output) => {
    if (err) {
      console.error(err);
    } else {
      createCombined(congress);
...
const createCombined = function (congress) {
  ...
  mapshaper.runCommands(mapshaperOptionsCombine.join(' '), (err, output) => {
    if (err) {
      console.error(err);
